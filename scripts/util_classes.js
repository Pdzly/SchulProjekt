...
export class usermanager {
  static getuserbyemail(email) {
    return new Promise((res, rej) => {
      fetch("/api/user/getuser", {
        ...
        body: JSON.stringify({ EMailAdresse: email }),
      })
        .then((val) => {
          return val.json();
        })
  ...
  static checkuser(username, pw) {
    return new Promise((res, rej) => {
      fetch("/api/user/verifyuser", {
        ...
        body: JSON.stringify({ Name: username, Passwort: pw.hash() }),
      })
        .then((val) => {
          return val.json();
        })
  ...
  static createuser(email, username, pw) {
    return new Promise((res, rej) => {
...
export class game {
  constructor(
    ...
    if (
      this.PlattformID &&
      (typeof this.PlattformID === "string" ||
        typeof this.PlattformID === "number")
    ) {
      if (typeof this.PlattformID === "number") {
        this.PlattformID = [this.PlattformID] ?? false;
      } else {
        this.PlattformID =
          this.PlattformID?.split("; ") ?? [this.PlattformID] ?? false;
      }
    }
    if (
      this.Plattform &&
      (typeof this.Plattform === "string" || typeof this.Plattform === "number")
    ) {
      if (typeof this.Plattform === "number") {
        this.Plattform = [this.Plattform] ?? false;
      } else {
        this.Plattform =
          this.Plattform?.split("; ") ?? [this.Plattform] ?? false;
      }
    }
    if (
      this.Genres &&
      (typeof this.Genres === "string" || typeof this.Genres === "number")
    ) {
    ...
    if (
      this.BildIDs &&
      (typeof this.BildIDs === "string" || typeof this.BildIDs === "number")
    ) {
    ...
    if (
      this.StudioID &&
      (typeof this.StudioID === "string" || typeof this.StudioID === "number")
    ) {
  ...
  addtoglide(glideclass) {
    ...
    if (Array.isArray(this.BildIDs) && this.BildIDs[0]) {
      bild.getBildById(this.BildIDs[0]).then(
        (val) => {
  ...
  parseStudios() {
    return new Promise((res, rej) => {
      ...
      this.StudioID.forEach((stid) => {
        promises.insert(
          0,
          studio.getstudiobyid(stid).then(
            (studios) => {
              if (studios) {
  ...
  parsePlattforms() {
    return new Promise((res, rej) => {
      ...
      this.PlattformID.forEach((plattfid) => {
        ...
        promises.insert(
          0,
          plattform.getplatformbyid(plattfid).then(
            (platt) => {
              if (platt) {
